...
class Describe_Row(object):
    ...
    def it_knows_its_index_in_table_to_help(self, idx_fixture):
        ...
        assert row._row_idx == expected_idx

    # fixtures -------------------------------------------------------

    @pytest.fixture
    def cells_fixture(self, _row_idx_, table_prop_, table_):
        row = _Row(None, None)
        _row_idx_.return_value = row_idx = 6
    ...
    @pytest.fixture
    def _row_idx_(self, request):
        return property_mock(request, _Row, '_row_idx')
